...
const AdminHome = () => {
    ...
    // Calculate total earnings and total profit based on filtered users
    const totalEarnings = filteredUsers.reduce(
        (total, users) => total + users.earnings,
        0,
    );
    const totalProfit = filteredUsers.reduce(
        (total, users) => total + users.profit,
        0,
    );
    ...
    // Chart data
    const chartData = {
        labels: ["Total Earnings", "Total Profit"],
        datasets: [
            {
                label: "Amount",
                data: [totalEarnings, totalProfit],
